...
def render_node_content(node, n2i, n2f, img):
    ...
    if ball_size:
        ...
        if bool(getattr(node, "_rasp_selection_ring", False)):
            ...
            ring_width = int(getattr(node, "_rasp_selection_ring_width", 3) or 3)
            ring_color = str(getattr(node, "_rasp_selection_ring_color", "#e02020") or "#e02020")
            selection_ring = QGraphicsEllipseItem(0, 0, ring_size, ring_size)
            ...
            selection_ring.setPen(ring_pen)
            selection_ring.setBrush(QBrush(Qt.NoBrush))
    ...
    for i in [selection_ring, node_ball, fblock_r, fblock_b, fblock_t]:
        if i:
