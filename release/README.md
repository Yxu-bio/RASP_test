...
python tools\build_engine_bundle.py build --channel internal
python tools\build_engine_bundle.py verify dist\RASP5-engine-bundle-2026.08.28-dev-windows-x86_64-internal.zip
```
...
5. Confirm that the root RASP5 MIT license and `CITATION.cff` are included;
   third-party engine licenses remain separate and are not replaced by it.
...
The Windows installer definition and reproducible staging workflow live under
`release/windows/` and `tools/stage_windows_release.py`. A locally verified
development installer exists, but publication still requires a clean source
tag, clean-machine acceptance, external-user reproduction, signing decision,
and final aggregate-license review.

`release/application-package.json` pins the reviewed Python runtime ZIP hash.
Staging emits `RASP5-STAGE-CONTENT.json`, which covers every immutable file
before conda relocation. Build Windows installers through
`tools/build_windows_installer.py`; it verifies that content manifest
immediately before invoking Inno Setup and writes the installer hash sidecar.
